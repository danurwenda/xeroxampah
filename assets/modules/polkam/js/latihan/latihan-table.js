jQuery(function ($) {
    ...
    $('#latihan-table').on('change', '.merge-cb', function (e) {
    ...
    $('#latihan-modal-form').on('show.bs.modal', function (e) {
        ...
        var modal = $(this)
                //tarik data dari db
                , fields = ['materi', 'motif', 'tempat','label'];//bisa gini soalnya semuanya tag input
        $.getJSON(base_url + 'latihan/get/' + selected[0], function (data) {
            ...
            if(data.sejak){
                $('.merge-1 input[name="sejak"]').combodate('setValue', data.sejak);
            }
            if(data.hingga){
                $('.merge-1 input[name="hingga"]').combodate('setValue', data.hingga);
            }
        });
        $.getJSON(base_url + 'latihan/get/' + selected[1], function (data) {
            fields.forEach(function (v, i) {
            ...
            //tanggal ngisinya beda
            if(data.sejak){
                $('.merge-2 input[nm="sejak"]').combodate('setValue', data.sejak);
            }
            if(data.hingga){
                $('.merge-2 input[nm="hingga"]').combodate('setValue', data.hingga);
            }
        });
    })
    $('#latihan-modal-form .btn-primary').click(function (e) {
        var form = $('#latihan-modal-form form')
                //serialize the form
                , h = form.serialize();
        // process the form
        $.ajax({
            type: 'POST', // define the type of HTTP verb we want to use (POST for our form)
            url: base_url + 'latihan/merge', // the url where we want to POST
