jQuery(function ($) {
    ...
    $('#school-modal-form').on('show.bs.modal', function (e) {
        ...
        var modal = $(this)
                //tarik data dari db
                , fields = ['school_name', 'address', 'label'];//bisa gini soalnya semuanya tag input
        $.getJSON(base_url + 'school/get/' + selected[0], function (data) {
        ...
        $.getJSON(base_url + 'school/get/' + selected[1], function (data) {
            fields.forEach(function (v, i) {
    ...
    $('#school-modal-form .btn-primary').click(function (e) {
        var form = $('#school-modal-form form')
                //serialize the form
                , h = form.serialize();
        // process the form
        $.ajax({
            type: 'POST', // define the type of HTTP verb we want to use (POST for our form)
            url: base_url + 'school/merge', // the url where we want to POST
    ...
    table.on('order.dt search.dt draw.dt', function () {
        ...
        table.rows().every(function (rowIdx, tableLoop, rowLoop) {
            var datax = this.data();
            if (selected.indexOf(datax.school_id) > -1)
    ...
    $(document).on(ace.click_event, '.action-buttons a.edit', function (e) {
        // popup warning
        var rowID = $(this).closest('tr').attr('id').substr(4);
        var win = window.open(base_url + 'school/edit/' + rowID, '_blank');
        win.focus();
    });
    ...
    $(document).on(ace.click_event, '.action-buttons a.view', function (e) {
        // popup warning
        var rowID = $(this).closest('tr').attr('id').substr(4);
        var win = window.open(base_url + 'graph/school/' + rowID, '_blank');
        win.focus();
    });
