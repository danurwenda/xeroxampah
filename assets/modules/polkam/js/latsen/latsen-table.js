jQuery(function ($) {
    //init datatable
    var table = $('#latsen-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: 'latsen/dt',
    ...
    $(document).on(ace.click_event, '.action-buttons a.edit', function (e) {
        // popup warning
        var rowID = $(this).closest('tr').attr('id').substr(4);
        var win = window.open(base_url + 'latsen/edit/' + rowID, '_blank');
        win.focus();
    });
    ...
    $(document).on(ace.click_event, '.action-buttons a.view', function (e) {
        // popup warning
        var rowID = $(this).closest('tr').attr('id').substr(4);
        var win = window.open(base_url + 'graph/latsen/' + rowID, '_blank');
        win.focus();
    });
